...
$(window).on('load', function() {
	...
	// Carreiras
	// -----------------------------------------------------------------
	var tabCarreiras = $('#tabCarreiras');
	tabCarreiras.footable().trigger('footable_collapse_all');


	$('#collapseCarreiras').on('click', function(){
		tabCarreiras.trigger('footable_collapse_all');
	});
	$('#expandCarreiras').on('click', function(){
		tabCarreiras.trigger('footable_expand_all');
	})
	// Search input
	$('#searchCarreiras').on('input', function (e) {
		e.preventDefault();
		tabCarreiras.trigger('footable_filter', {filter: $(this).val()});
	});

	// Cursos
	// -----------------------------------------------------------------
	var tabCursos = $('#tabCursos');
	tabCursos.footable().trigger('footable_collapse_all');


	$('#collapseCursos').on('click', function(){
		tabCursos.trigger('footable_collapse_all');
	});
	$('#expandCursos').on('click', function(){
		tabCursos.trigger('footable_expand_all');
	})
	// Search input
	$('#searchCursos').on('input', function (e) {
		e.preventDefault();
		tabCursos.trigger('footable_filter', {filter: $(this).val()});
	});
